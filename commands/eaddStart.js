  ...
  const createdEmote = await message.guild.emojis.create(emoteLink, emoteName, {reason: `emote created by ${message.author.tag} thru command line`}).catch(error => {
    if (error.code == 50035) {
      ...
      return message.reply(toobigEmbed).then(() => {
        console.log("emote creation fail, restarting")
        process.exit()
      });
      ...
      return message.channel.send("something went wrong with making this emote (unknown error), just make sure that everything is correct, do `-eadd` and read through it pls\nthis keeps happening even thought u checked everything? spam ameer lol").then(() => {
        console.log("emote creation fail, restarting")
        process.exit()
      });
    }
