...
module.exports.run = async (bot, message, args) => {
    ...
    const commandNameEmbed = new Discord.MessageEmbed()
        ...
        .addField("what's this?", "name you command now, this is not what the bot will say, but what will trigger the bot to say something.")
    ...
    const responseEmbed = new Discord.MessageEmbed()
        ...
        .addField("whats this?", "enter here what do you want this bot to respond with when the command is entered, can be anything as long as it's sensible")
        .addField("`reminder`", "you have 30 seconds to answer this or the proccess will be nulled and you'll have to start again!")
        .setTimestamp()
        .setColor(color)
        .setFooter(message.author.tag + " | " + bot.user.username, message.author.displayAvatarURL({dynamic: true, size: 1024}))
    ...
    if(commandName.includes("@")) {
        return message.channel.send("you can't use the symbol `@` as it can be used to mention members and that's a nono\nread <#773800772219568169> pleas")
    }
    ...
    let commandStringChecker = commandString.toLowerCase()
    if(commandStringChecker.includes("allahu akbar")) {
        message.delete()
        return message.reply("let's just avoid that one, pick something else!, here is a link to the dictionary: https://www.dictionary.com/ ")
    }

    if(commandString.includes("@" || "@everyone" || "@here")) {
        return message.channel.send("you can't use the symbol `@` as it can be used to mention members and that's a nono\nread <#773800772219568169> please")
    }
